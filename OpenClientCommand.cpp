...
void OpenClientCommand::runClient() {
  ...
  while (!VariableMap::getBool()) {
    ...
    try {
      ...
      int varAmount, i;
      queue<pair<string, float>> updateQueue = VariableMap::getUpdatedVars();
      if (!updateQueue.empty()) {
        for (i = 0; i < updateQueue.size(); i++) { // going through the vars is the list
          //mtxC.lock();
          string sim = updateQueue.front().first;
          float val = updateQueue.front().second;
          //mtxC.unlock();
      ...
      VariableMap::getInstanceVarsMap()->clearUpdatedVars();
