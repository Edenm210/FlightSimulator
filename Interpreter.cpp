                    ...
                    if (i == (s.length() - 1)) { throw "Error: operator at the end of the expression"; }
                        ...
                        if (s[i + 1] == '*' || s[i + 1] == '/' || s[i + 1] == '+' || s[i + 1] == '-') {
                            ...
                            if (i == 0) {
                                if (s[i] == '*'
                                    || s[i] ==
                                       '/') { throw "Error: Binary operator can not be at the start of the expression"; }
                                ...
                                if (s[i - 1] == '(') {
                                    ...
                                    while (!stack.empty()
                                           && precedence(s[i]) <=
                                              precedence(stack.top())) // as long as the precedes is higher in the stack
                                    {
                                        str += stack.top();// changing the char to string
                                        postfix_queue.push(str);
                                        stack.pop();
    ...
    for (unsigned int i = 0; i <= variables_string.length(); i++) {
        ...
        if (equation_side.compare("left") == 0) {
            if (variables_string[i] == '-') {
                ...
                if ((checkType(variables_string[i]) != 3) && !isdigit(variables_string[i]) &&
                    variables_string[i] != '=') {
